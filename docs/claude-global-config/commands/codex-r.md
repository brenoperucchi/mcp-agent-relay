---
description: Envia uma revisão de código pro Codex via MCP agent relay (dispatch_wait), usando gpt-5.6-sol/high, e devolve o veredito. Roda no worker em background, sem websocket direto.
...
2. **Despache e aguarde** com `mcp__agentrelay__dispatch_wait`:
   ...
   - `task`: objeto JSON com `prompt` (a instrução do passo 1),
     `model: "gpt-5.6-sol"`, `effort: "high"`. **O worker só lê
