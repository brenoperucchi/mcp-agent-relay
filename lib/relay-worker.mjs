...
import { getExecutor } from "./executor-registry.mjs";
...
export async function processJob(
  ...
  const resolvedExecutor = executor ?? getExecutor(agentId);
  if (write && resolvedExecutor?.allowWrites === false) {
    const fr = fail(cwd, job.id, claimToken, "Claude não permite escrita nesta versão");
    if (!fr.ok) return { outcome: "lost", job: getJob(cwd, job.id) };
    return { outcome: "failed", reason: "claude_write_denied", job: getJob(cwd, job.id) };
  }
...
export async function drainOnce(
  ...
  const executor = getExecutor(agentId);
  if (executor?.ok === false) return null;
  const selectedRunTurn = runTurn === defaultRunTurn ? executor.runTurn : runTurn;
  ...
  for (const candidate of inbox) {
    ...
    if (claimed) {
      return processJob(cwd, claimed.job, claimed.claimToken, { runTurn: selectedRunTurn, leaseMs, heartbeatMs, allowWrites, agentId, executor, signal });
    }
  }
  return null;
}
...
export async function dispatchAndWait(
  ...
  if (claimed) {
    ...
    const jobDone = processJob(cwd, claimed.job, claimed.claimToken, {
      runTurn: runTurn === defaultRunTurn ? executor.runTurn : runTurn,
      leaseMs,
      heartbeatMs,
      allowWrites,
      agentId: to,
