...
export async function processJob(
  ...
  if (controller.signal.aborted) {
    ...
    if (write) {
      const settled = park(cwd, job.id, claimToken, "abortado por timeout (write) — needs_recovery");
  ...
  const settled = fail(cwd, job.id, claimToken, result?.error ?? "turn failed");
  if (!settled.ok) return { outcome: "lost", job: getJob(cwd, job.id) };
  return { outcome: "failed", job: getJob(cwd, job.id) };
}
  ...
  while (!signal?.aborted) {
    ...
    if (processed) {
      ...
      if (idleTimeoutMs != null) {
        ...
        if (Date.now() - idleSince >= idleTimeoutMs) break;
      }
