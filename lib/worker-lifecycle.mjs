...
async function _ensure(cwd, opts) {
  ...
  if (ready) {
    ...
    // Keep the child referenced until readiness settles. Node 20 can otherwise
    // let the parent event loop finish before a timeout path observes `close`.
    child.unref();
    return result;
  }
  ...
  if (intent === "kill") {
    ...
    // Another process adopted this child while we were waiting; it is now a
    // daemon from this process's perspective too.
    child.unref();
  }
